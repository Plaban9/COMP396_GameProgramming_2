...
public class NPCController_Modified_PB : MonoBehaviour
{
    ...
    private static void D(string message, bool isError = false)
    {
        if (isError)
        {
            Debug.LogError($"<<NPCController_Modified_PB>> {message}");
            return;
        }

        Debug.Log($"<<NPCController_Modified_PB>> {message}");
    }
